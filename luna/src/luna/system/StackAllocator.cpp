...
namespace luna {
    ...
    StackAllocator::Marker StackAllocator::allocAligned(size_t bytes, size_t alignment) {
        ...
        return static_cast<Marker>(pAlignedMem);
    }
    
    void StackAllocator::freeAligned(Marker mem) {
        const uint8_t* pAlignedMem = reinterpret_cast<uint8_t*>(mem);
        
        uintptr_t alignedAddress = reinterpret_cast<uintptr_t>(mem);
        uintptr_t adjustment = static_cast<uintptr_t>(pAlignedMem[-1]);
        
        uintptr_t rawAddress = alignedAddress - adjustment;
        Marker pRawMem = reinterpret_cast<Marker>(rawAddress);
        
        freeToMarker(pRawMem);
    }
